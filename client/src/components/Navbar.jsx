...
const Navbar = () => {
    ...
    // Register socket and listen for DMs (for unread count badge)
    useEffect(() => {
        if (socket && user?._id) {
            socket.emit('register-user', user._id);
            const onDmReceive = () => {
                if (!dmOpen) setUnreadDMs(n => n + 1);
            };
            socket.on('dm-receive', onDmReceive);
            return () => socket.off('dm-receive', onDmReceive);
        }
    }, [socket, user?._id, dmOpen]);
    ...
    // Explore replaced by People
    const navLinks = ['Home', 'People', 'Insights', 'Community'];
    ...
    return (
        <>
                ...
                <div className="px-4 sm:px-8">
                    <div className="flex justify-between items-center h-16">
                        ...
                        <div className="hidden md:flex items-center gap-3">
                            ...
                            {/* DM Button — only for logged in users */}
                            {user && (
                                <button
                                    onClick={() => openDM()}
                                    className="relative p-2 text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-full transition-colors"
                                    title="Messages"
                                >
                            ...
                            {user ? (
                                ...
                                <>
                                    ...
                                    <Link to="/register" className="btn-primary px-6 py-2.5 rounded-full text-sm font-medium shadow-lg shadow-indigo-500/20">Sign Up Free</Link>
                                </>
                            )}
                        </div>
                        ...
                        <div className="md:hidden flex items-center gap-3">
                            <button onClick={toggleDarkMode} className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors">
                                {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
                            </button>
                            {user && (
                                <button onClick={() => openDM()} className="relative p-2 text-gray-500 dark:text-gray-400 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                    <MessageSquare size={20} />
                                    {unreadDMs > 0 && (
                                        <span className="absolute -top-0.5 -right-0.5 bg-indigo-600 text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                                            {unreadDMs}
                            ...
                            {user && (
                                <Link to="/profile" className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center text-indigo-600 dark:text-indigo-300 font-bold text-sm border border-indigo-200 dark:border-indigo-700">
                                    {user.avatar ? <img src={user.avatar} alt={user.username} className="w-full h-full object-cover rounded-full" /> : user.username.charAt(0).toUpperCase()}
                                </Link>
                            )}
                            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors">
                            ...
                            <div className="px-6 py-6 flex flex-col gap-4">
                                {navLinks.map((item) => {
                                    ...
                                    return (
                                        <Link
                                            key={item}
                                            to={path}
                                            className={`px-4 py-3 rounded-xl text-base font-medium transition-all ${active ? 'bg-indigo-50 text-indigo-600' : 'text-gray-600 hover:bg-gray-50'}`}
                                        >
                                            {item}
                                        </Link>
                                    );
                                ...
                                {!user ? (
                                    <div className="flex flex-col gap-3">
                                        <Link to="/login" className="w-full py-3 text-center text-gray-600 font-medium hover:bg-gray-50 rounded-xl transition-colors">Log In</Link>
                                        <Link to="/register" className="btn-primary w-full py-3 text-center rounded-xl font-medium shadow-lg shadow-indigo-500/20">Sign Up Free</Link>
                                    </div>
                                ) : (
                                    <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors w-full">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" />
                                        </svg>
                                        Sign Out
            ...
            {/* DM Drawer — rendered at top level so it overlays everything */}
            <DMDrawer
                isOpen={dmOpen}
                onClose={() => setDmOpen(false)}
                initialContact={dmContact}
            />
        </>
    );
};
